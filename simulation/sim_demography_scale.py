...
| 3  | AA  | simulated African American |
...
parser.add_argument("sample_n_AA",help="Sample size of AA", type=int)
args = parser.parse_args()

def out_of_africa(sample_n_AF, sample_n_EU, sample_n_AS, sample_n_AA):
    ...
    N_AA = N_AA0 / math.exp(-r_AA * T_admixture)
    ...
    population_configurations = [
        ...
        msprime.PopulationConfiguration(
            sample_size=sample_n_AA, initial_size=N_AA, growth_rate=r_AA)
    ]
    migration_matrix = [
        [      0, m_AF_EU, m_AF_AS, 0],
        [m_AF_EU,       0, m_EU_AS, 0],
    ...
    demographic_events = [
        ...
        msprime.MassMigration(
            time=T_admixture, source=3, destination=0, proportion=0.8),
        msprime.MassMigration(
            time=T_admixture+0.0001, source=3, destination=1, proportion=1),
        msprime.PopulationParametersChange(
            time=T_admixture, growth_rate=0, population_id=3),
...
outofafrica_tree = out_of_africa(args.sample_n_AF, args.sample_n_EU, args.sample_n_AS, args.sample_n_AA)
